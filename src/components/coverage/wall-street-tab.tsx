...
import type {
  ...
  WallStreetHistoryEntry,
  WallStreetReportDetail,
} from "@/types/coverage";
...
function isWallStreetFirms(x: unknown): x is WallStreetFirmCoverage[] {
  ...
  return x.every(
    (f) =>
      ...
      typeof (f as WallStreetFirmCoverage).blurb === "string" &&
      Array.isArray((f as WallStreetFirmCoverage).history)
  );
}

function reportRowKey(firmId: string, h: WallStreetHistoryEntry): string {
  return h.entryId ?? `${firmId}::${h.date}::${h.action}`;
}

function WallStreetReportDetailPanel({
  detail,
}: {
  ...
  return (
    <div className="mt-8 space-y-10 border border-border/60 bg-muted/20 px-6 py-10 sm:px-10">
      <section>
        <h4 className={sectionLabel}>{detail.aiSummaryTitle}</h4>
        <div className="mt-4 max-w-2xl space-y-4 text-sm leading-[1.65] text-foreground/90">
          {detail.aiSummaryParagraphs.map((p, i) => (
            <p key={i}>{p}</p>
          ))}
        </div>
      </section>
        ...
        <section>
          ...
          <ul className="mt-4 space-y-3 text-sm leading-relaxed text-foreground/90">
            {detail.catalysts.map((c, i) => (
              <li key={`c-${i}`} className="flex gap-3">
                <span
...
export function WallStreetTab({ ticker }: { ticker: string }) {
  ...
  /** At most one report’s research detail open at a time (accordion). */
              ...
              <div className="mt-12">
                ...
                <div className="mt-4 overflow-x-auto">
                  <table className="w-full min-w-[640px] border-collapse text-left text-sm">
                    ...
                    <tbody>
                      {firm.history.map((h) => {
                        const rk = reportRowKey(firm.id, h);
                        const hasDetail = Boolean(h.detail);
                        const expanded = openReportKey === rk;
                        const toggleId = `wall-street-toggle-${rk.replace(/[^a-zA-Z0-9-]/g, "-")}`;
                        const panelId = `wall-street-detail-${rk.replace(/[^a-zA-Z0-9-]/g, "-")}`;
                              ...
                              <td className="py-4 align-top">
                                ...
                                {hasDetail && h.detail ? (
                                  <button
                                    ...
                                    className="group mt-4 inline-flex items-center gap-2 border-b border-transparent pb-0.5 text-left text-sm font-medium text-foreground transition-colors hover:border-foreground/40"
                                  >
                                    {expanded ? (
                              ...
                              <tr className="border-b border-border/50 bg-muted/15">
                                <td colSpan={5} className="p-0 align-top">
                                  ...
                                  >
                                    ...
                                    >
                                      Research detail — {firm.firm} — {h.date}{" "}
                                      · {h.action}
                                    </p>
                                    <WallStreetReportDetailPanel
                                      detail={h.detail}
                                    />
                                  </div>
                                </td>
